...
class AttendancesController < ApplicationController
  ...
  def create
    ...
    if !Attendance.find_by(month: @attendance.month, employee_id: @attendance.employee_id)
      if authorised_admin? && @attendance.save
        @attendance.update(working_days: find_working_days(@attendance.id),
                           unpaid_leaves: find_total_unpaid_leaves(@attendance.id))
  ...
  def find_total_unpaid_leaves(id)
    ...
    leaves = Leave.where('extract(month from from_date) = ?',
                         @attendance.month).where(leave_type: 'unpaid').where(employee_id: @attendance.employee_id)
    @total_leaves = 0
